...
- `tests/bhumi.tcyr` — test cases. Use `assert_eq` / `assert` and exit with `assert_summary()`.
- `dist/bhumi.cyr` — single-file bundle produced by `cyrius distlib`. Consumers `include` this from their own `cyrius.cyml [deps.bhumi] modules = ["dist/bhumi.cyr"]`.

## Adding a feature

1. Edit `src/main.cyr` (or add a new module and `include` it).
2. Add a test case to `tests/bhumi.tcyr`.
3. Run `cyrius test`.
